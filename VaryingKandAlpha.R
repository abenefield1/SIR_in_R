...
#parameters=data.frame(rec=0.2,trans=0.003, death=0.1,birth=0.1, mut=0.05, alpha=0)
#k=2
PopulationSize<-1000
...
#det = function(k, a=parameters$alpha, dMax=1){
#  return(dMax*exp(-k*a))
#}
...
initial_states <- build_data(n_k=2,total_pop_size = PopulationSize)
kvec<-seq(2,5,by=1)
res2 <- vector(length(kvec),mode="list")
for (b in seq_along(kvec)){ #range of values for beta
  initial_states <- build_data(n_k=kvec[b],total_pop_size = PopulationSize)
  res <- vector(length(alphavec),mode="list")
  for (o in seq_along(alphavec)){ #range of values for alpha
    parameters=data.frame(rec=0.2,trans=0.003, death=0.1,birth=0.1, mut=0.05, a=alphavec[o])
    det = function(k, a=parameters$a, dMax=1){
      return(dMax*exp(-k*a))
    }
    sir_modelAW(time, initial_states, parameters)
    
    res[[o]] <- ode(y=initial_states,
                    func = sir_modelAW,
                    parms=parameters,
                    times = time, 
                    method="ode45")
  ...
  names(res)<-alphavec
  dd <- dplyr::bind_rows(lapply(res,as.data.frame),.id="a")
  dd$a <- as.numeric(dd$a)
  res2[[b]]<-dd
}
  names(res2)<-kvec
  dd2 <- dplyr::bind_rows(lapply(res2,as.data.frame),.id="n_k")
  dd2$n_k <- as.numeric(dd2$n_k)
  
finalStates<-  lapply(res2, function(x) { subset(x, time > 99)} )  
finals<- dplyr::bind_rows(lapply(finalStates,as.data.frame),.id="n_k")
finals$n_k <- as.numeric(finals$n_k)
finalsLong=melt(finals,id=c("a","n_k","time"))
...
ggplot(data = out_long,                                               
       aes(x = time, y = value, colour = a, group = variable)) +  
  geom_line() +                                                          
  xlab("Time (years)")+                                                  
...
finalsLong$n_k<-as.numeric(finalsLong$n_k)

ggplot(finalsLong, aes(x = n_k, y = a, fill = value)) +
